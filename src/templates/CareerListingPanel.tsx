...
export const CareerListingPanel = ({ onBack, onManageJobs }: { onBack: () => void, onManageJobs: () => void }) => {
  // --- UPDATED: Use user and hasPermission from AuthContext ---
  ...
  // --- UPDATED: Dynamic Permission Checks ---
  const canEditStatus = hasPermission("career:edit") || user?.role === "SuperAdmin";
  const canManageJobs = hasPermission("career:jobs_manage") || user?.role === "SuperAdmin";
  const canExport = hasPermission("career:export") || user?.role === "SuperAdmin";
  ...
  const fetchData = async (page = 1) => {
    ...
    try {
      ...
      // Optimized: Using relative path as axiosInstance handles baseURL
  ...
  const updateStatus = async (e: React.ChangeEvent<HTMLSelectElement>, id: string) => {
    e.stopPropagation();
    if (!canEditStatus) {
      toast.error("Insufficient permissions to update status");
      return;
    }
    try {
  ...
  return (
    <div className="min-h-screen bg-white p-8 relative">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between mb-12 gap-6">
          ...
          <div className="flex flex-col md:flex-row items-center gap-4">
            <div className="text-center">
              ...
              <h2 className="text-3xl font-bold text-slate-900 bg-green-50 px-4 py-2 rounded-2xl inline-block">{totalSuccesses}</h2>
            </div>
            <div className="text-center">
              <p className="text-xs font-black uppercase tracking-[0.2em] text-slate-400 mb-1">Total Failures</p>
              <h2 className="text-3xl font-bold text-slate-900 bg-red-50 px-4 py-2 rounded-2xl inline-block">{totalFailures}</h2>
            </div>
        ...
        {/* Table View */}
        <div className="overflow-hidden border border-slate-100 rounded-[2.5rem] shadow-sm bg-white">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-50">
              {loading ? (
                <tr><td colSpan={6} className="py-24 text-center text-slate-400 animate-pulse font-bold tracking-widest text-xs uppercase">Syncing Applications...</td></tr>
              ) : data.length === 0 ? (
                <tr><td colSpan={6} className="py-24 text-center text-slate-400 italic">No records found matching filters.</td></tr>
              ) : data.map((app) => (
                <tr key={app._id} onClick={() => fetchAppDetails(app._id)} className="group hover:bg-blue-50/30 cursor-pointer transition-all">
                  ...
                  <td className="px-8 py-6 text-xs font-bold text-slate-400">
                    {app.processingStatus ? app.processingStatus.toUpperCase() : "COMPLETED"}
                  </td>
            ...
            <div className="p-10 md:p-14 text-slate-900">
              <div className="mb-10">
                ...
                <div className="flex flex-wrap gap-6 mt-6 text-slate-500 text-sm font-bold">
                   ...
                   <span className="flex items-center gap-2 text-slate-700"><HiOutlinePhone className="text-blue-500" /> {selectedApp.mobile}</span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-12 border-t border-slate-100 pt-10">
                <div className="space-y-8">
                  <section>
                    <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-4">Professional Info</h4>
