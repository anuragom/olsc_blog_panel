...
export const PickupListingPanel = ({ onBack }: { onBack: () => void }) => {
  ...
  // --- NEW: DIRECT TABLE REMARKS UPDATE WITH FEEDBACK ---
  const handleTableRemarkUpdate = async (item: PickupRequest, newValue: string) => {
    const originalValue = item.remarks || "";
    if (newValue === originalValue) return;
    if (!canEditStatus) return; // Reusing pickup:edit for remarks authorization
    ...
    try {
      ...
      toast.success(
        <div className="flex flex-col">
          ...
          <span className="text-xs opacity-80">{item.consignor_fullName} ({item.consignor_pinCode})</span>
        </div>,
        { id: `pickup-save-${item._id}` }
      );

      setTimeout(() => setSuccessId(null), 2000);
    } catch (err) {
      toast.error(`Failed to save remarks for ${item.consignor_fullName}`);
    } finally {
      setSavingId(null);
    }
  };
                ...
                <tr key={item._id} onClick={() => setSelectedPickup(item)} className="group hover:bg-blue-50/20 cursor-pointer transition-all">
                  ...
                  <td className="px-8 py-6" onClick={e => e.stopPropagation()}>
                    {canEditStatus ? (
                      ...
                      <p className="text-xs text-slate-500 italic font-medium truncate max-w-[200px]">{item.remarks || "No remarks"}</p>
                    )}
                  </td>
                </tr>
              ))}
          ...
          <div className="relative w-full max-w-2xl bg-white h-full shadow-2xl animate-in slide-in-from-right duration-500 overflow-y-auto border-l border-slate-100">
            <div className="p-12">
              ...
              <div className="space-y-10">
                ...
                {/* Section: Remarks for Detail View */}
                <div className="bg-emerald-50/50 p-8 rounded-[2rem] border border-emerald-100 shadow-inner">
                  <h4 className="text-[10px] font-black uppercase tracking-widest text-emerald-600 mb-4 flex items-center gap-2">Official Log Note</h4>
                  <p className="text-sm font-bold text-slate-700 leading-relaxed italic">
                    {selectedPickup.remarks || "No operational remarks recorded."}
                  </p>
                </div>
