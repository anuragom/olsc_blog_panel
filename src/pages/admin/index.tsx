...
// //   HiOutlineShare, HiOutlineDatabase, HiOutlineLink
// // } from "react-icons/hi";
...
// //               { id: "service:retail-fashion:read", label: "Retail Fashion: View" },
// //               { id: "service:retail-fashion:write", label: "Retail Fashion: Edit", dependsOn: ["service:retail-fashion:read", "enquiry:edit"] },
...
// //   const togglePermission = (id: string, dependsOn?: string[], dependsOn_OR?: string[]) => {
...
// //       const isSelected = selectedPerms.includes(node.id);
...
// //             onClick={() => hasChildren ? setExpandedGroups(prev => prev.includes(node.id) ? prev.filter(g => g !== node.id) : [...prev, node.id]) : togglePermission(node.id, node.dependsOn, node.dependsOn_OR)}
// //           >
// //             {hasChildren ? (
...
// //    * This ensures the canvas follows the same indentation as the tree
// //    */
// //   const renderCanvasTree = (nodes: any[], level = 0) => {
// //     return nodes.map(node => {
// //       const isSelected = selectedPerms.includes(node.id);
// //       const hasSelectedChild = node.children?.some((child: any) => 
// //         selectedPerms.includes(child.id) || child.children?.some((gc: any) => selectedPerms.includes(gc.id))
// //       );

// //       // We only show this node on the canvas if it or its children are selected
// //       if (!isSelected && !hasSelectedChild) return null;

// //       return (
// //         <div key={`canvas-${node.id}`} className="relative">
// //           <div 
// //             className={`flex items-center gap-4 mb-4 transition-all duration-500 animate-in slide-in-from-left`}
...
// //             <div className={`p-4 rounded-2xl flex-1 border backdrop-blur-md transition-all ${isSelected ? 'bg-white/10 border-white/20' : 'bg-white/5 border-white/5 opacity-40'}`}>
// //                 <div className="flex items-center justify-between">
// //                     <p className="text-[10px] font-black text-blue-300 uppercase tracking-widest">{node.children ? 'Branch' : 'Permission'}</p>
// //                     {isSelected && <HiOutlineLink className="text-blue-400" size={12} />}
// //                 </div>
// //                 <p className="text-sm font-bold text-white leading-tight mt-1">{node.label}</p>
// //             </div>
// //           </div>
// //           {node.children && renderCanvasTree(node.children, level + 1)}
// //         </div>
// //       );
// //     });
// //   };
...
// //             <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar border-t border-slate-50 pt-4">
// //               {renderTree(PERMISSION_TREE)}
// //             </div>
// //             <button 
// //   onClick={async () => {
// //     // Change: Check and return separately
// //     if (!roleName) {
// //       toast.error("Enter Role Name");
// //       return; // This ensures the return value is consistent (void)
// //     }
    
// //     try {
// //       await axiosInstance.post("/auth/roles", { 
// //         name: roleName, 
...
// //     }
// //   }} 
// //   className="mt-6 w-full py-5 bg-[#0D5BAA] text-white rounded-[2rem] font-black uppercase text-xs shadow-xl shadow-blue-200"
// // >
// //   Save Architecture
// // </button>
// //           </div>

// //           {/* Column 2: Visual Canvas with FIXED INDENTATION */}
...
// //                         <h3 className="text-xs font-black uppercase tracking-[0.4em] text-blue-400">Live Access Canvas</h3>
// //                         <p className="text-[10px] text-white/30 font-medium mt-1">Hierarchical visualization of active gates</p>
// //                     </div>
// //                     <HiOutlineShare className="text-white/20" />
// //                 </div>

// //                 <div className="flex-1 overflow-y-auto custom-scrollbar-white pr-4">
// //                     {selectedPerms.length === 0 ? (
// //                         <div className="h-full flex flex-col items-center justify-center text-white/20 border-2 border-dashed border-white/5 rounded-[3rem]">
// //                             <HiOutlineDatabase size={60} className="mb-4" />
// //                             <p className="text-[10px] font-black uppercase tracking-widest">Awaiting selection...</p>
// //                         </div>
// //                     ) : (
// //                         <div className="space-y-2">
// //                            {renderCanvasTree(PERMISSION_TREE)}
// //                         </div>
// //                     )}
// //                 </div>
...
// //                     <div>
// //                         <p className="text-[9px] font-black text-white/40 uppercase tracking-widest mb-1">Authorization Matrix</p>
// //                         <p className="text-2xl font-black text-white">{selectedPerms.length} <span className="text-xs text-white/50 uppercase">Nodes Active</span></p>
// //                     </div>
// //                     <div className="w-12 h-12 rounded-full bg-[#EE222F] flex items-center justify-center shadow-2xl shadow-red-500/50">
...
// //               {roles.map((r: any) => (
// //                 <div key={r._id} className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm hover:border-[#0D5BAA] transition-all group">
// //                    <p className="font-black text-slate-900 text-xs uppercase mb-1">{r.name}</p>
// //                    <p className="text-[9px] font-bold text-slate-400 uppercase">{r.permissions.length} Logic Gates</p>
// //                 </div>
// //               ))}
// //             </div>
// //           </div>
// //         </div>
// //       ) : (
// //         /* User Assignment View remains the same */
// //         <div className="max-w-2xl mx-auto bg-white p-16 rounded-[4rem] border border-slate-100 shadow-sm text-center">
// //             <h2 className="text-3xl font-black text-slate-900 tracking-tighter uppercase mb-2">Protocol Assignment</h2>
// //             <p className="text-xs font-medium text-slate-400 mb-12 uppercase tracking-widest">Update operator identity protocols</p>
// //             <div className="relative mb-12">
// //                 <input value={empSearch} onChange={e => setEmpSearch(e.target.value)} placeholder="EMPLOYEE ID..." className="w-full p-6 bg-slate-50 rounded-[2rem] font-black outline-none border border-slate-100 uppercase" />
// //                 <button onClick={async () => {
// //                   try {
// //                     const res = await axiosInstance.get(`/auth/users/${empSearch}`);
// //                     setFoundUser(res.data.user);
// //                   } catch (e) { toast.error("Operator not found"); }
...
// //                         <p className="text-2xl font-black text-slate-900">{foundUser.fullName}</p>
// //                         <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest">{foundUser.role?.name || 'Standard Access'}</p>
// //                     </div>
// //                   </div>
// //                   <select value={targetRoleId} onChange={e => setTargetRoleId(e.target.value)} className="w-full p-6 bg-slate-50 rounded-2xl mb-10 font-black text-slate-700 outline-none ring-1 ring-slate-100">
// //                     <option value="">-- SELECT NEW PROTOCOL --</option>
// //                     {roles.map((r: any) => <option key={r._id} value={r._id}>{r.name.toUpperCase()}</option>)}
// //                   </select>
// //                   <button onClick={async () => {
// //                     try {
// //                       await axiosInstance.patch(`/auth/users/${foundUser._id}/role`, { roleId: targetRoleId });
// //                       toast.success("Protocol Updated"); setFoundUser(null);
// //                     } catch (e) { toast.error("Failure"); }
// //                   }} className="w-full py-7 bg-[#0D5BAA] text-white rounded-[2.5rem] font-black uppercase text-xs shadow-xl shadow-blue-200">Commit Changes</button>
// //                </div>
// //             )}
// //         </div>
// //       )}
...
// import axiosInstance from "@/utils/axiosInstance";
// import { 
//   HiOutlineShieldCheck, HiOutlineUserAdd, HiOutlineSearch, 
//   HiOutlineChevronDown, HiOutlineChevronRight, HiCheckCircle,
//   HiOutlineShare, HiOutlineDatabase, HiOutlineLink, HiX,
//   HiOutlineEye
// } from "react-icons/hi";
// import { toast } from "react-hot-toast";
...
//   // Modal state for viewing role details
...
//             </div>
//             <div className={`p-4 rounded-2xl flex-1 border backdrop-blur-md transition-all ${isSelected ? 'bg-white/10 border-white/20' : 'bg-white/5 border-white/5 opacity-40'}`}>
//                 <p className="text-[9px] font-black text-blue-300 uppercase tracking-[0.2em] mb-1">{node.children ? 'Module' : 'Permission'}</p>
...
//                     <HiOutlineShare className="text-white/20" />
//                 </div>
//                 <div className="flex-1 overflow-y-auto custom-scrollbar-white pr-4 text-left">
//                     {selectedPerms.length === 0 ? (
//                         <div className="h-full flex flex-col items-center justify-center text-white/20 border-2 border-dashed border-white/5 rounded-[3rem]">
//                             <HiOutlineDatabase size={60} className="mb-4" />
//                             <p className="text-[10px] font-black uppercase tracking-widest">Awaiting selection...</p>
//                         </div>
//                     ) : (
//                         <div className="space-y-2">{renderCanvasTree(PERMISSION_TREE)}</div>
//                     )}
//                 </div>
//                 <div className="mt-8 pt-6 border-t border-white/5 flex items-end justify-between text-left">
//                     <div>
//                         <p className="text-[9px] font-black text-white/40 uppercase tracking-widest mb-1">Authorization Matrix</p>
//                         <p className="text-2xl font-black text-white">{selectedPerms.length} <span className="text-xs text-white/50 uppercase">Nodes</span></p>
//                     </div>
...
//           {/* Column 3: Existing Registry (Clickable Title) */}
//           <div className="xl:col-span-3 flex flex-col h-full">
//             <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-6 mb-6">Existing Registry</h3>
//             <div className="flex-1 overflow-y-auto space-y-4 pr-2 custom-scrollbar">
//               {roles.map((r: any) => (
...
//                   className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm hover:border-[#0D5BAA] transition-all group cursor-pointer active:scale-95"
//                 >
//                    <div className="flex items-center justify-between mb-2">
//                        <p className="font-black text-slate-900 text-xs uppercase group-hover:text-[#0D5BAA] transition-colors">{r.name}</p>
//                        <HiOutlineEye className="text-slate-300 group-hover:text-[#0D5BAA]" size={16} />
//                    </div>
//                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{r.permissions.length} Logic Gates Active</p>
//                 </div>
//               ))}
//             </div>
//           </div>
...
//       {/* --- RIGHT SIDE ROLE DETAILS SLIDING MODAL --- */}
//       {viewingRole && (
//         <div className="fixed inset-0 z-[100] flex justify-end">
//           <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-300" onClick={() => setViewingRole(null)}></div>
...
//             <div className="p-12">
//               <button onClick={() => setViewingRole(null)} className="mb-10 p-3 bg-slate-50 rounded-2xl hover:bg-red-50 hover:text-red-500 transition-all shadow-sm"><HiX size={24} /></button>
              
//               <div className="mb-12">
//                 <p className="text-[10px] font-black text-[#0D5BAA] uppercase tracking-[0.4em] mb-4 italic">Protocol Manifest</p>
//                 <h2 className="text-4xl font-black text-slate-900 tracking-tighter uppercase">{viewingRole.name}</h2>
//                 <div className="mt-4 flex items-center gap-2">
//                     <div className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse"></div>
//                     <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Active System Integration</p>
//                 </div>
//               </div>
...
//                     <div className="p-8 bg-[#001F39] rounded-[2.5rem] text-white shadow-xl">
//                         <p className="text-[9px] font-black uppercase tracking-widest text-blue-400 mb-2">Logic Gates</p>
//                         <p className="text-4xl font-black">{viewingRole.permissions.length}</p>
//                     </div>
//                     <div className="p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100 flex flex-col justify-center">
//                         <p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-2">Creation Node</p>
//                         <p className="text-xs font-bold text-slate-700">DB_VER_12.04</p>
//                     </div>
//                 </div>
...
//                         <HiOutlineShieldCheck className="text-[#0D5BAA]" size={20} />
//                         <h4 className="text-xs font-black uppercase tracking-widest text-slate-400">Authorized Access Nodes</h4>
//                     </div>
                    
//                     <div className="grid grid-cols-1 gap-3">
//                         {viewingRole.permissions.map((perm: string) => (
//                             <div key={perm} className="flex items-center gap-4 bg-slate-50 p-5 rounded-3xl border border-slate-100 group hover:border-blue-200 transition-all">
//                                 <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-[#0D5BAA] shadow-sm ring-1 ring-slate-100">
//                                     <HiOutlineLink size={18} />
//                                 </div>
//                                 <div className="flex flex-col">
//                                     <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{perm.split(':')[0]}</p>
//                                     <p className="text-sm font-bold text-slate-800 uppercase">{findLabel(perm)}</p>
//                                 </div>
//                             </div>
//                         ))}
//                     </div>
...
//                   className="w-full py-6 bg-slate-900 text-white rounded-[2.5rem] font-black uppercase tracking-[0.2em] text-xs shadow-xl shadow-slate-200 hover:bg-[#0D5BAA] transition-all mt-10"
//                 >
//                   Close Record View
//                 </button>
//               </div>
