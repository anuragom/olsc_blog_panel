...
export default function EditBlogPage() {
  ...
  useEffect(() => {
    if (!slug) return;
    setLoading(true);
    axiosInstance.get(`${baseUrl}/blogs/slug/${slug}`)
      .then((res) => {
        const blog = res.data;
        const normalizedBlocks = (blog.blocks || []).map((b: any) => ({
          id: b.id ?? uuidv4(),
  ...
  }, [slug, currentWebsite, baseUrl]);

  if (loading || !initialData)
    return <div className="flex h-screen w-full items-center justify-center font-bold">Loading Editor...</div>;
